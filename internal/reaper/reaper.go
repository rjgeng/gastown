...
func parentExcludeJoin(dbName string) (joinClause, whereCondition string) {
	joinClause = `LEFT JOIN (
		...
		LEFT JOIN wisps pw ON pw.id = COALESCE(wd.depends_on_wisp_id, wd.depends_on_issue_id)
		LEFT JOIN issues pi ON pi.id = COALESCE(wd.depends_on_issue_id, wd.depends_on_wisp_id)
...
func Scan(db *sql.DB, dbName string, maxAge, purgeAge, mailDeleteAge, staleIssueAge time.Duration) (*ScanResult, error) {
	...
	staleQuery := `
		...
		AND i.id NOT IN (
			...
			INNER JOIN issues dep ON dep.id = COALESCE(d.depends_on_issue_id, d.depends_on_wisp_id, d.depends_on_external)
...
func AutoClose(db *sql.DB, dbName string, staleAge time.Duration, dryRun bool) (*AutoCloseResult, error) {
	...
	whereClause := fmt.Sprintf(`
		...
		AND i.id NOT IN (
			...
			INNER JOIN `+"`%s`"+`.issues dep ON dep.id = COALESCE(d.depends_on_issue_id, d.depends_on_wisp_id, d.depends_on_external)
			WHERE dep.status IN ('open', 'in_progress')
		)
		AND i.id NOT IN (
...
func batchDeleteRows(ctx context.Context, db *sql.DB, idQuery string, cutoffArg time.Time, primaryTable string, auxTables []string) (int, error) {
	...
	for {
		...
		delReverse := fmt.Sprintf("DELETE FROM wisp_dependencies WHERE depends_on_wisp_id IN %s OR depends_on_issue_id IN %s", inClause, inClause)
		doubleArgs := append(args, args...)
		if _, err := db.ExecContext(ctx, delReverse, doubleArgs...); err != nil {
			// Non-fatal.
		}
